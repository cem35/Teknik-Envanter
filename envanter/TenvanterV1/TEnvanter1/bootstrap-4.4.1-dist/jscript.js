...
$(document).ready(function(){
 $.each(response, function(i, item) {
    	...
    	if(item.position == "Team Leader")
	    {
	    	counter++;
	    	var $tr = $('<tr>').append(
	            $('<td> ').html('<a data-toggle="collapse" href="#collapse'+counter+'" class="oncol" id="collapse"> + </a> ' + item.name ),
        ...
        {
        	var $tr = $('<tr id="collapse'+counter+'"  class="panel-collapse collapse">').append(
	            $('<td>').html(item.name),
	            $('<td>').html('<a href="#ex1" rel="modal:open" id="popupOpener">'+ item.tckn +'</a>'),
	            $('<td>').html('<a href="tel:+9'+item.phone+'">'+item.phone+'</a>')
	        ); 
        ...
        //var append = window.document.getElementById("records_table").append($tr);
